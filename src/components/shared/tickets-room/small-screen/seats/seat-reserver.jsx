...
export default function SeatsReserver() {
  ...
  const totalStandar =
    standard.length !== 0 ? standard.length * standard[0].giaVe : 0;

  const premium = selectedSeats.filter((seat) => seat.loaiGhe !== "Thuong");
  const totalPremium =
    premium.length !== 0 ? premium.length * premium[0].giaVe : 0;

  const total =
    selectedSeats.length !== 0 ? totalStandar + totalPremium : "null";
